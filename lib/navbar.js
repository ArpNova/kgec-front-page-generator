          ...
          <ul class="navbar-nav ms-auto gap-2 align-items-lg-center">
            ...
            <li class="nav-item dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-end p-2" style="width: 320px;">
                ...
                <div id="nav-subjects-list" class="d-flex flex-column gap-1 custom-scroll" style="max-height: 280px; overflow-y: auto;"></div>
              </div>
            </li>
          </ul>
        </div>
...
function renderNavSubjectsList(subjects, filterText = "") {
  ...
  filtered.forEach(([code, name]) => {
    ...
    row.className = "d-flex gap-1 subject-row";
    row.innerHTML = `
      <button type="button" class="btn btn-sm copy-code btn-subject-code" style="flex: 0 0 40%;">${code}</button>
      <button type="button" class="btn btn-sm copy-name btn-subject-name text-start" style="flex: 1;">${name}</button>
    `;
